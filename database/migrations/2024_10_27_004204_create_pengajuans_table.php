...
class PengajuanController extends Controller
{
    ...
    public function markAsCompleted($id)
    {
        try {
            $pengajuan = Pengajuan::findOrFail($id);
            $pengajuan->status = 'completed'; // or your appropriate status
            $pengajuan->save();

            return response()->json(['message' => 'Pengajuan marked as completed.']);
        } catch (ModelNotFoundException $e) {
            return response()->json(['message' => 'Pengajuan not found'], 404);
        } catch (QueryException $e) {
            Log::error('Database error: ' . $e->getMessage(), ['exception' => $e]);
            return response()->json(['message' => 'Database error occurred while updating status'], 500);
        } catch (Exception $e) {
            Log::error('General error: ' . $e->getMessage(), ['exception' => $e]);
